describe("Ebupot Unifikasi - Rekam Bupot", () => {
  ...
  it("switches company and opens Rekam Bukti Potong", () => {
    ...
    const getNextDocNumber = () => {
      ...
      return cy.wrap(`TEST/CYPRESS/${y}${m}${d}-${hh}${mm}`);
    };
    ...
    getNextDocNumber().then((docNumber) => {
      createdDocs.push(docNumber);
      ...
      // assertion value
      cy.get('input[name="tanggal-pemotongan"]').should(
        "have.value",
        "01 Januari 2026"
      );
      ...
      const pilihNitku = (value) => {
        ...
        cy.get("div.dropdown-content:visible")
          .should("be.visible")
          .within(() => {
            cy.get('input[placeholder="Cari"]').clear().type(value);
      ...
      const uploadDraftByNumber = (docNumber) => {
        cy.get('input[placeholder="Cari Nomor Dokumen"]')
          .should("be.visible")
          .clear()
          .type(docNumber);

        cy.contains("td", docNumber).should("be.visible");
        cy.get("#button-row-options-0").click({ force: true });

        cy.contains("li.option-item", "Upload Bupot").click();
        cy.get("#button-confirm-action").click();
        cy.contains("Permintaan Upload Bupot Berhasil").should("be.visible");
      };

      const uploadDraftLast = () => {
        const lastDoc = createdDocs[createdDocs.length - 1];
        if (!lastDoc) {
          throw new Error("Belum ada draft yang dibuat untuk di-upload.");
        }
        uploadDraftByNumber(lastDoc);
      };

      // ===== SINGLE UPLOAD DRAFT TERAKHIR =====
      uploadDraftLast();
    });
  });
});
